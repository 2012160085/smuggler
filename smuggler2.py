...
class smuggler:    
    ...
    def pickBits(self):
        ...
        if self.bit_buffer.size == 0 :
            return []
        value = self.bit_buffer[:self.img_space]
        self.bit_buffer = self.bit_buffer[self.img_space:]
        self.bytes_written = self.bytes_written + self.modulation/8
        value = value.reshape(len(self.img_array),len(self.img_array[0]),4,self.modulation)
        return value
